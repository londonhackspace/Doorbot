...
DOCUMENT_TYPE= {DOC_UNDEF:'Undefined',\
		DOC_ID:'ID Card',\
		DOC_PASS:'Passport',\
		DOC_PASS2:'Passport',\
		DOC_PASS3:'Passport',\
...
def decode_ef_dg1(data):
	...
	print '    Document type: %s' % DOCUMENT_TYPE[DocumentType]
	if DocumentType == DOC_ID:
...
if not FILES and not TEST:
	...
	# 08 = 818 kBaud
	if not passport.hsselect('08'):
		print 'Could not select passport! (%s)' % passport.errorcode
		os._exit(True)
...
for tag in eflist:
	...
	if not FILES:
		...
		try:
			...
			print "Can't open %s" % filespath+TAG_FILE[tag]
			os._exit(True)
		data= passfile.read()
...
if Jmrtd:
	...
	if STRIP_INDEX:
		...
		while i-2 < total_length-1:
			...
			if tag=='5C'.decode('hex'):
				...
				clearDGs=[chr(0x61), chr(0x75), chr(0x67), chr(0x6b), chr(0x6c), chr(0x6d)]
	...
	jmrtd_personalise(passport.MRPnumber,passport.MRPdob,passport.MRPexpiry)
	print "JMRTD/vonJeek 3DES key set to: "+passport.MRPnumber+passport.MRPdob+passport.MRPexpiry
if JmrtdLock:
	jmrtd_lock()
...
if not Nogui:
	...
	if Filetype == "JP2":
		...
		Filetype= 'JPG'
	# display data according to document type
	if DocumentType == DOC_ID:
